...
x = LpVariable.dicts("x", [(i,j1,j2)for i in I for j1 in V1 for j2 in V2], cat='Binary')
...
#)
model += (
    lpSum(d1[(i,j1)]* x[(i,j1,j2)] for i in I for j1 in V1 for j2 in V2) +
    lpSum(d2[(j1,j2)]* x[(i,j1,j2)] for i in I for j1 in V1 for j2 in V2) +
...
# محدودیت‌ها
for i in I:
    model += lpSum(x[(i,j1,j2)] for j1 in V1 for j2 in V2) == 1

for i in I:
    for j2 in V2:
        model += lpSum(x[(i,j1,j2)] for j1 in V1) <= Y2[j2]

for i in I:
    for j1 in V1:
        model += lpSum(x[(i,j1,j2)] for j2 in V2) <= Y1[j1]
